...
const swaggerDirectory = join(projectDirectory, "docs/wb/swagger");
const metadata = readJson("docs/wb/swagger.meta.json");
const wbSourceDirectory = join(projectDirectory, "packages/wb/src");
const outputPath = join(wbSourceDirectory, "api.generated.ts");
const endpointsDirectory = join(wbSourceDirectory, "endpoints");
const contractsDirectory = join(wbSourceDirectory, "contracts");
const manifestPath = join(projectDirectory, "docs/wb/endpoints.json");
const referencePath = join(projectDirectory, "docs/wb/API-REFERENCE.md");
const checkOnly = process.argv.includes("--check");
const HTTP_METHODS = new Set(["get", "post", "put", "patch", "delete"]);

const files = readdirSync(swaggerDirectory)
  .filter((file) => file.endsWith(".yaml"))
  .sort();
if (JSON.stringify(files) !== JSON.stringify(metadata.files)) {
  throw new Error("docs/wb/swagger.meta.json does not match WB Swagger files.");
}

const documents = files.map((file) => {
  const source = readFileSync(join(swaggerDirectory, file), "utf8");
  const document = YAML.parse(source);
  const sourceName = document.info?.["x-file-name"];
  if (typeof sourceName !== "string") {
    throw new Error(`Missing info.x-file-name in ${file}.`);
  }
  return {
    file,
    source,
    document,
    sourceName,
    domain: toCamelCase(sourceName),
    domainTitle: document.info?.title ?? sourceName,
    documentation:
      sourceName === "general"
        ? "https://dev.wildberries.ru/docs/openapi/api-information"
        : `https://dev.wildberries.ru/docs/openapi/${sourceName}`,
  };
});
...
const operations = documents.flatMap((entry) =>
  collectOperations(entry).map((operation) => {
    if (operationNames.has(operation.methodName)) {
      throw new Error(
        `Duplicate generated WB method: ${operation.methodName}.`,
      );
    }
...
if (operations.length !== 286) {
  throw new Error(`Expected 286 WB operations, found ${operations.length}.`);
}
for (const operation of operations) {
  operation.deprecation = createWbDeprecation(operation, operations);
}
...
for (const [path, source] of renderGeneratedFiles(
  ...
  generatedFiles.set(path, await format(source, { parser: "typescript" }));
}
...
if (checkOnly) {
  for (const [path, source] of generatedFiles) {
    checkFile(path, source, "pnpm update:wb");
  }
  checkGeneratedFileSet(generatedFiles);
  checkFile(manifestPath, manifestSource, "pnpm update:wb");
  checkFile(referencePath, referenceSource, "pnpm update:wb");
  console.log(`WB generated API is current (${operations.length} operations).`);
} else {
  ...
  console.log(`Generated WB API for ${operations.length} operations.`);
}
...
function readRequestBody(operation, document) {
  ...
  throw new Error(
    `Unsupported WB request content type: ${Object.keys(content).join(", ")}.`,
  );
}
...
function renderGeneratedFiles(entries, allOperations, aliases) {
  const files = new Map([
    ...
    [
      join(wbSourceDirectory, "values.generated.ts"),
      renderWbValues(entries, allOperations),
    ],
  ]);
...
function renderWbValues(entries, allOperations) {
  const valueSets = collectWbValueSets(entries, allOperations);
  const lines = [
    "/**",
  ...
  for (const valueSet of valueSets) {
    ...
    for (const value of valueSet.values) {
      let propertyName = value.length === 0 ? "Empty" : toPascalCase(value);
...
function collectWbValueSets(entries, allOperations) {
  const candidates = [];
  for (const entry of entries) {
...
function renderApiRegistry(entries, allOperations, aliases) {
  ...
  const hostEntries = productionOrigins.map((origin) => {
    const hostname = new URL(origin).hostname.replace(/\.wildberries\.ru$/, "");
    return [toPascalCase(hostname.replace(/-api$/, "")), origin];
  });
  const lines = [
  ...
  lines.push(
    "",
    `export const WbApiHost = ${JSON.stringify(Object.fromEntries(hostEntries))} as const;`,
    "export type WbApiHost = (typeof WbApiHost)[keyof typeof WbApiHost];",
    `export const WB_API_ORIGINS = ${JSON.stringify(origins)} as const;`,
  ...
  for (const entry of entries) {
    ...
    lines.push(`export interface Wb${toPascalCase(entry.domain)}Api {`);
  ...
  lines.push("export interface WbDomainApi {");
  for (const entry of entries) {
    lines.push(
      `  readonly ${entry.domain}: Wb${toPascalCase(entry.domain)}Api;`,
    );
  }
  lines.push(
    "}",
  ...
  lines.push(
    `export const WB_OPERATION_METADATA = ${JSON.stringify(metadataRows)} as const;`,
    "",
    `export const WB_OPERATION_ALIASES = ${JSON.stringify(
      Object.fromEntries(
        entries.map((entry) => [
          entry.domain,
          Object.fromEntries(
...
function renderComponentRegistry(entry) {
  ...
  return [
    ...
    "",
    `export const ${entry.domain}Components: WbSchemaComponents = ${JSON.stringify(components)};`,
    "",
  ].join("\n");
}
...
function renderEndpointTypes(operation) {
  ...
  const types = [inputType, responseType || "undefined"];
  return [
    "// Generated public types for this Wildberries endpoint.",
    ...(types.some((type) => type.includes("WbJsonValue"))
    ...
    renderJsDoc([
      ...
      ...operation.responses.map(
        (response) =>
          `${response.status}: ${response.description || "успешный ответ"}.`,
      ),
    ]),
...
function renderExpandedSchemaType(node, entry, stack = []) {
  ...
  if (stack.includes(node.name)) {
    throw new Error(
      `Recursive WB schema ${[...stack, node.name].join(" -> ")} in ${entry.file}.`,
    );
  }
  const rawSchema = entry.document.components?.schemas?.[node.name];
  if (rawSchema === undefined) {
    throw new Error(`Unknown WB component ${node.name} in ${entry.file}.`);
  }
  let result = renderExpandedSchemaType(
    normalizeSchema(rawSchema, entry.document),
    entry,
    [...stack, node.name],
  );
  if (node.nullable === true) result = `(${result}) | null`;
  return result;
}
...
function renderEndpointContract(operation) {
  ...
  const parserImports = [
    ...(hasBinary ? ["parseWbBinaryResponse"] : []),
    ...(hasJson ? ["parseWbResponse"] : []),
    ...(!hasDefaultResponse ? ["unexpectedWbSuccessStatus"] : []),
  ];
  const lines = [
  ...
  for (const response of operation.responses) {
    ...
    lines.push(
      `const ${schemaName} = createWbSchema<${responseType}>(${JSON.stringify(runtimeSchemaNode(response.schema))}, ${operation.entry.domain}Components);`,
    );
  }
  lines.push(
    "",
  ...
  lines.push(
    ...
    `      ${
      defaultResponse === undefined
        ? `return unexpectedWbSuccessStatus(${JSON.stringify(operation.operationId)}, status);`
...
function renderEndpointExecute(operation) {
  const lines = [
    ...
    'import type { WbOperationDefinition, WbOperationExecutor } from "../../../operation.js";',
    'import type { WbRequestOptions } from "../../../request-options.js";',
    ...
    "",
    `const definition: WbOperationDefinition<${operation.typeName}Response> = {`,
...
function renderEndpointBinding(operation) {
  if (operation.inputSections.length === 0) {
    return `(options: WbRequestOptions = {}) => execute${operation.typeName}(execute, options)`;
  }
  if (operation.inputRequired) {
    return `(input: ${operation.typeName}Input, options: WbRequestOptions = {}) => execute${operation.typeName}(execute, input, options)`;
  }
  return `(input?: ${operation.typeName}Input, options: WbRequestOptions = {}) => execute${operation.typeName}(execute, input, options)`;
}
...
function renderGeneratedSource(entries, allOperations) {
  ...
  const lines = [
    ...
    "",
    'import type { WbOperationDefinition, WbOperationExecutor } from "./operation.js";',
    'import { parseWbBinaryResponse, parseWbResponse, unexpectedWbSuccessStatus } from "./parse-response.js";',
    'import { createWbSchema, type WbJsonValue, type WbSchemaComponents } from "./schema.js";',
    'import type { WbRequestOptions } from "./request-options.js";',
    "",
    `export const WbApiHost = ${JSON.stringify(Object.fromEntries(hostEntries))} as const;`,
    "export type WbApiHost = (typeof WbApiHost)[keyof typeof WbApiHost];",
  ...
  for (const entry of entries) {
    ...
    lines.push(
      `const ${entry.domain}Components: WbSchemaComponents = ${JSON.stringify(components)};`,
      "",
    );
  }
...
function renderOperationDefinition(operation) {
  ...
  for (const response of operation.responses) {
    ...
    schemaNames.set(response.status, schemaName);
    schemaLines.push(
      `const ${schemaName} = createWbSchema<${responseType}>(${JSON.stringify(response.schema)}, ${operation.entry.domain}Components);`,
    );
  }
  const definitionName = `${operation.methodName}Definition`;
  const lines = [
    ...schemaLines,
    `const ${definitionName}: WbOperationDefinition<${responseType}> = {`,
...
function renderResponseParser(operation, response, schemaNames) {
  ...
  if (response.kind === "binary") {
    return `return parseWbBinaryResponse(input, ${JSON.stringify(operation.operationId)});`;
  }
  return `return parseWbResponse(${schemaNames.get(response.status)}, input, ${JSON.stringify(operation.operationId)});`;
}
...
function renderMethodDeclaration(operation) {
  const documentation = renderJsDoc([
    ...
    operation.tags.length === 0
      ? ""
      : `Разделы WB: ${operation.tags.join(", ")}.`,
...
function renderMethodSignature(operation) {
  if (operation.inputSections.length === 0) {
    return `(options?: WbRequestOptions) => Promise<${operation.typeName}Response>`;
  }
  return `(input${operation.inputRequired ? "" : "?"}: ${operation.typeName}Input, options?: WbRequestOptions) => Promise<${operation.typeName}Response>`;
}

function renderMethodImplementation(operation) {
  const definition = `${operation.methodName}Definition`;
  if (operation.inputSections.length === 0) {
    return `(options: WbRequestOptions = {}) => execute(${definition}, undefined, options)`;
  }
  if (operation.inputRequired) {
    return `(input: ${operation.typeName}Input, options: WbRequestOptions = {}) => execute(${definition}, input, options)`;
  }
  return `(input: ${operation.typeName}Input = {}, options: WbRequestOptions = {}) => execute(${definition}, input, options)`;
}
...
function renderSchemaType(node, entry) {
  ...
  switch (node.kind) {
    ...
    case "ref":
      result =
        entry === undefined
          ? `__WB_COMPONENT_${Buffer.from(node.name).toString("hex")}__`
...
function renderReference(entries, allOperations, aliases) {
  const lines = [
    ...
    "",
    `Сгенерировано из ${entries.length} локальных официальных OpenAPI-файлов. Проверено: ${metadata.verifiedAt}.`,
    "",
    `Всего операций: **${allOperations.length}**.`,
    "",
    `Рекомендуемых алиасов без версии: **${aliases.length}**. Версионные методы остаются доступны для явной фиксации контракта.`,
    "",
  ];
  for (const entry of entries) {
    const domainOperations = allOperations.filter(
    ...
    for (const operation of domainOperations) {
      ...
      lines.push(
        `| ${alias === undefined ? "—" : `\`wb.${operation.entry.domain}.${alias}(...)\``} | \`wb.${operation.entry.domain}.${operation.methodName}(...)\` | ${escapeMarkdownTable(operation.summary)}${operation.deprecation === undefined ? "" : `<br>**Устарел:** ${escapeMarkdownTable(operation.deprecation)}`} | ${operation.method} | \`${operation.path}\` | [execute.ts](../../packages/wb/src/endpoints/${endpointDirectory}/execute.ts), [types.ts](../../packages/wb/src/endpoints/${endpointDirectory}/types.ts), [contract.ts](../../packages/wb/src/endpoints/${endpointDirectory}/contract.ts) |`,
      );
    }
    lines.push("");
  }
...
function createWbDeprecation(operation, allOperations) {
  if (!operation.deprecated) return undefined;

  const replacements = allOperations.filter(
    (candidate) =>
      candidate !== operation && operation.description.includes(candidate.path),
  );
  if (replacements.length === 0) {
    throw new Error(
      `Deprecated WB method ${operation.methodName} has no documented replacement implemented in the SDK.`,
    );
  }

  const label = replacements.length === 1 ? "Замена в SDK" : "Замены в SDK";
  const methods = replacements
    .map(
      (replacement) =>
        `\`wb.${replacement.entry.domain}.${replacement.methodName}(...)\` для \`${replacement.method} ${replacement.path}\``,
    )
    .join("; ");
  return `Метод помечен устаревшим в OpenAPI WB. ${label}: ${methods}.`;
}
...
function collectVersionlessAliases(allOperations) {
  ...
  for (const candidates of candidatesByAlias.values()) {
    ...
    if (ranked.length > 1 && ranked[0].version === ranked[1].version) {
      throw new Error(
        `Ambiguous WB versionless alias ${ranked[0].operation.entry.domain}.${ranked[0].alias}.`,
      );
    }
...
function renderJsDoc(parts) {
  const text = unique(
    ...
    .join("\n\n")
    .replaceAll("*/", "* /");
  if (text.length === 0) return "/** Описание отсутствует в OpenAPI WB. */";
  return ["/**", ...text.split("\n").map((line) => ` * ${line}`), " */"].join(
    "\n",
  );
}
...
function checkGeneratedFileSet(expectedFiles) {
  ...
  if (extra.length > 0) {
    throw new Error(`${extra[0]} is obsolete. Run pnpm update:wb.`);
  }
}
