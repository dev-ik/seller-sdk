...
export interface OzonRequestOptions {
  ...
  /** Количество повторов безопасного запроса после первой попытки. */
...
function validateRequestOptions(options: OzonRequestOptions): void {
  ...
  if (issues.length > 0) {
    throw new ConfigurationError("Invalid Ozon request options.", issues);
  }
}
