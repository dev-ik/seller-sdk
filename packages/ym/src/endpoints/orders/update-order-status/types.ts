...
type OrdersOrderStatusChangeDTO = {
  readonly status: OrdersOrderStatusType;
  readonly substatus?: OrdersOrderSubstatusType;
  readonly delivery?: OrdersOrderStatusChangeDeliveryDTO;
};
type OrdersOrderStatusChangeDeliveryDTO = {
  readonly dates?: OrdersOrderStatusChangeDeliveryDatesDTO;
};
type OrdersOrderStatusChangeDeliveryDatesDTO = {
...
type OrdersUpdateOrderStatusRequest = {
  readonly order: OrdersOrderStatusChangeDTO;
};
type OrdersUpdateOrderStatusResponse = {
  readonly order?: OrdersOrderDTO;
  readonly operation?: OrdersOperationDTO;
};

/**
 * Параметры метода «Изменение статуса одного заказа».
 *
...
export type UpdateOrderStatusInput = {
   ...
   * Тело запроса.
   */;
  readonly body: OrdersUpdateOrderStatusRequest;
};
/**
 * Успешный ответ метода «Изменение статуса одного заказа».
 *
 * 200: В случае успешного изменения статуса заказа возвращается обновленная информация о заказе.
 */
export type UpdateOrderStatusResponse = OrdersUpdateOrderStatusResponse;
