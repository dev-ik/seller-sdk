...
export interface YmParameterDefinition {
...
export interface YmOperationDefinition<TResponse> {
  readonly operationId: string;
  readonly method: HttpMethod;
  readonly path: string;
  readonly productionOrigin: string;
  readonly sandboxOrigin?: string;
  readonly parameters: readonly YmParameterDefinition[];
...
export type YmOperationExecutor = <TInput, TResponse>(
  definition: YmOperationDefinition<TResponse>,
  input: TInput | undefined,
  options: YmRequestOptions,
) => Promise<TResponse>;

export interface YmOperationExecutionContext {
  readonly getTransport: (origin: string) => Transport;
}

/** @internal */
export function createYmOperationExecutor(
  context: YmOperationExecutionContext,
): YmOperationExecutor {
  return async <TInput, TResponse>(
    ...
    const origin = definition.productionOrigin;
...
function mapRequest<TInput, TResponse>(
  ...
  if (
    ...
    throw new ConfigurationError(
      `Yandex Market multipart body must be a FormData instance for operation ${definition.operationId}.`,
    );
  }
...
function toInputRecord(
  ...
  throw new ConfigurationError(
    `Yandex Market operation ${operationId} expects an input object.`,
  );
}
...
function readSection(
  ...
  throw new ConfigurationError(
    `Yandex Market operation ${operationId} expects ${key} to be an object.`,
  );
}

function missingParameterError(
  operationId: string,
  parameter: YmParameterDefinition,
): ConfigurationError {
  return new ConfigurationError(
    `Missing required Yandex Market ${parameter.location} parameter ${parameter.name} for operation ${operationId}.`,
    [
      {
        code: "required",
...
function invalidParameterError(
  ...
  return new ConfigurationError(
    `Invalid Yandex Market ${parameter.location} parameter ${parameter.name} for operation ${operationId}.`,
    [
      {
        code: "invalid_type",
        path: [parameter.location, parameter.name],
