...
import type { GetV1QuestionsResponse } from "./types.js";

const getV1QuestionsResponse200Schema = createWbSchema<GetV1QuestionsResponse>(
  {
    kind: "object",
    properties: {
      data: {
...
export function parseGetV1QuestionsResponse(
  status: number,
  input: unknown,
  ...
  switch (status) {
    case 200:
      return parseWbResponse(
        getV1QuestionsResponse200Schema,
        input,
        "getV1Questions",
      );
    default:
      return unexpectedWbSuccessStatus("getV1Questions", status);
  }
}
