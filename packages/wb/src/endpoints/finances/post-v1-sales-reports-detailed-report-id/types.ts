...
/**
 * Параметры метода «Детализации к отчётам реализации по ID отчётов».
 *
 ...
 */
export type PostV1SalesReportsDetailedReportIdInput = {
  /**
   * Параметры пути.
   */
  readonly path: {
    /**
     * ID отчёта.
     * Для ежедневных отчётов вместо стандартной десериализации рекомендуем использовать нестандартные библиотеки с поддержкой [BigInt](https://www.npmjs.com/package/json-bigint)
     *
     * Формат: `int64`.
     */
...
/**
 * Успешный ответ метода «Детализации к отчётам реализации по ID отчётов».
 *
 * 200: Успешно.
 *
 * 204: Нет данных.
 */
export type PostV1SalesReportsDetailedReportIdResponse =
  | readonly {
      /**
       * ID отчёта
       *
